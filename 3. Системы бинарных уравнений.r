...
# Тема 3. Системы бинарных уравнений
# --------
...
# 2.1. Добавьте в уравнение занятости переменную на
...
# 2.2. Повторите предыдущее задание заменив уравнение
...
lnL <- function(par, y, X)
{
  ...
  y_li <- X %*% beta
  L <- rep(NA, n)
  L[y == 1] <- pt(y_li[y == 1], df = 5)
  L[y == 0] <- 1 - pt(y_li[y == 0], df = 5)
  
  return(sum(log(L)))
}
lnL(c(1, 0.01, 0.01, 0.01), y = h$default, X = cbind(1, h$age, log(h$income), h$educ_1))
...
opt <- optim(par = x0,
             ...
             control = list(maxit = 10000,
                            ...
                            fnscale = -1),
             y = h$default, 
             X = cbind(1, h$age, log(h$income), h$educ_1))
opt$par
opt$value
model_probit <- glm(formula = default ~ age +      
                                        log(income) + educ_1,
                    data = h,                             
                    family = binomial(link = "probit"))
