...
# Тема 6. Модель Тобина и усеченная регрессия
# --------
...
#---------------------------------------------------
# Часть 1. Оценивание параметров модели с
#          с использованием усеченной регрессии
#          и модели Тобина
#---------------------------------------------------

# -----
# Учимся:
# 1. Оценивать коэффициенты при регрессорах с
#    использованием усеченной регрессии
# 2. Оценивать коэффициенты при регрессорах с
#    использованием модели Тобина
# -----
...
# --------------------------------------------
# Описание переменных:
# credit         - объем выданного кредита
# age            - возраст
# income         - индекс здоровья
...
#---------------------------------------------------
# Часть 2. Самостоятельная реализация модели Тобина
#---------------------------------------------------

# -----
# Учимся:
# 1. Программировать функцию правдоподобия
#    модели Тобина
# 2. Максимизировать функцию правдоподобия
#    модели Тобина при помощи методов
#    численной оптимизации
# -----

# Запишем функцию правдоподобия
# модели Тобина
lnL <- function(x, y, X, tr_left, tr_right)
{
  ...
  L_vector[cond_1] <- pnorm(y[cond_1] - XB[cond_1,],     # считаем вклады в
                            mean = 0, sd = sigma)        # функцию правдоподобия
  L_vector[cond_2] <- 1 - pnorm(y[cond_2] - XB[cond_2,], 
                                mean = 0, sd = sigma)
  L_vector[cond_3] <- dnorm(y[cond_3] - XB[cond_3,], 
                            mean = 0, sd = sigma)
  
  lnL <- sum(log(L_vector))                              # считаем логарифм
...
# 1.3*.   Реализуйте модель Тобина со случайными ошибками,
#         имеющими распределение Стьюдента с 5-ю степенями
#         свободы
 
#---------------------------------------------------
# Часть 3. Расчет предсказаний и предельных эффектов
#---------------------------------------------------
...
epsilon_E <- truncatedNormalMoment(k = 1,                # условное математическое        
               ...
               mean = 0, sd = sigma_tobit)               # = E(e|lower - xb < e < upper - xb) 
